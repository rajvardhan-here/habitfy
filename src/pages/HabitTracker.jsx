...
const T = {
  ...
  dark: {
    ...
    amber:'#FCD34D', shadow:'rgba(0,0,0,0.4)',
  }
}

// ─── WaterBowl ────────────────────────────────────────────────────────────────
function WaterBowl({ percent, t }) {
        ...
        <svg viewBox="0 0 110 110" style={{position:'absolute',top:0,left:0,width:'100%',height:'100%'}}>
          <defs>
            ...
            <linearGradient id="waterGrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%"   stopColor={t.pink}   stopOpacity={0.8}/>
          ...
          <g clipPath="url(#bowlClip)">
            ...
            <motion.path
              ...
              fill={t.purple} opacity={0.3} animate={{x:[0,50,0]}} transition={{duration:2.5,repeat:Infinity,ease:'linear'}}/>
          </g>
          <ellipse cx="55" cy="58" rx="46" ry="46" fill="none" stroke={t.purple} strokeWidth="2.5"/>
          <ellipse cx="55" cy="13" rx="46" ry="9"  fill={t.purpleLight} stroke={t.purple} strokeWidth="2"/>
          <ellipse cx="55" cy="13" rx="38" ry="6"  fill="white" stroke={t.purple} strokeWidth="1.5" opacity="0.8"/>
          <text x="55" y="60" textAnchor="middle" style={{fontSize:17,fontWeight:900,fill:percent>50?'white':t.text}}>{percent}%</text>
          <text x="55" y="74" textAnchor="middle" style={{fontSize:8,fill:percent>50?'rgba(255,255,255,0.8)':'#94A3B8'}}>this month</text>
        </svg>
        {isOverflow&&(<>
          <motion.div style={{position:'absolute',top:2,left:32,width:7,height:14,borderRadius:'50%',background:t.pink,opacity:0.8}}
            animate={{y:[0,-8,0],scaleY:[1,1.3,1]}} transition={{duration:1,repeat:Infinity}}/>
          <motion.div style={{position:'absolute',top:0,left:52,width:6,height:12,borderRadius:'50%',background:t.purple,opacity:0.7}}
...
// ─── MiniCircle ───────────────────────────────────────────────────────────────
function MiniCircle({ percent, t }) {
  const r=9, circ=2*Math.PI*r, offset=circ-(percent/100)*circ
  return (
    <svg width={22} height={22} style={{flexShrink:0}}>
      <circle cx={11} cy={11} r={r} fill="none" stroke={t.purpleLight} strokeWidth={3}/>
      <circle cx={11} cy={11} r={r} fill="none" stroke={t.pink} strokeWidth={3}
...
function StreakPill({ streak, t }) {
  ...
  return (
    <div style={{position:'relative'}}
      ...
      onTouchStart={()=>setTip(v=>!v)}>
      <div style={{display:'flex',alignItems:'center',gap:6,padding:'6px 16px',borderRadius:20,
        background:t.card,boxShadow:`0 2px 10px ${t.shadow}`,cursor:'default',userSelect:'none'}}>
        <span style={{fontSize:13}}>🔥</span>
        <span style={{fontWeight:700,color:t.purple,fontSize:12}}>Streak</span>
        ...
        {tip&&(
          <motion.div initial={{opacity:0,y:6,scale:0.95}} animate={{opacity:1,y:0,scale:1}}
            ...
            style={{position:'absolute',top:46,left:'50%',transform:'translateX(-50%)',
              background:t.text,color:t.bg,borderRadius:14,padding:'12px 16px',
              fontSize:11.5,fontWeight:600,lineHeight:1.7,zIndex:200,
              boxShadow:`0 8px 24px ${t.shadow}`,width:230,textAlign:'center'}}>
            <div style={{position:'absolute',top:-6,left:'50%',transform:'translateX(-50%)',
              width:0,height:0,borderLeft:'6px solid transparent',borderRight:'6px solid transparent',
              borderBottom:`6px solid ${t.text}`}}/>
...
// ─── Main HabitTracker ────────────────────────────────────────────────────────
export default function HabitTracker({ user: propUser, onLogout, dark, onToggleDark }) {
  const t = dark ? T.dark : T.light
  ...
  const addHabit=async()=>{
    ...
    const cu=await getCU(); if(!cu) return
    const {data,error}=await supabase.from('habits').insert({user_id:cu.id,name:newHabit,color:t.pink}).select()
    if(error){console.error(error);return}
    setHabits(prev=>[...prev,{...data[0],habit_logs:[]}]); setNewHabit('')
  }
  ...
  const saveEdit=async(habit)=>{
    ...
    await supabase.from('habits').update({name:editName}).eq('id',habit.id)
    setHabits(prev=>prev.map(h=>h.id===habit.id?{...h,name:editName}:h))
  ...
  const getHabitMonthProgress=(habit)=>{
    if(!habit.habit_logs||!habit.habit_logs.length) return 0
  ...
  return (
    <div style={{minHeight:'100vh',background:t.bg,fontFamily:'Inter, sans-serif',overflowY:'auto',transition:'background 0.3s'}}>
      ...
      <div style={{padding:isMobile?12:20,paddingTop:16}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 160px 220px',gap:14,marginBottom:16}}>
          ...
          <div style={{...C,padding:14,height:isMobile?200:220,display:'flex',flexDirection:'column'}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:6,flexShrink:0}}>
              <div style={{display:'flex',alignItems:'center',gap:8}}>
                <h3 style={{fontWeight:700,fontSize:13,color:t.text,margin:0}}>Progress</h3>
                <span style={{fontSize:11,fontWeight:600,color:t.pink}}>{MONTHS[selectedMonth]}</span>
              </div>
              <div style={{position:'relative'}}>
                <button onClick={()=>setShowMonthPick(!showMonthPick)}
                  style={{fontSize:10,fontWeight:500,color:t.purple,background:t.purpleLight,
                    padding:'3px 10px',borderRadius:20,border:'none',cursor:'pointer'}}>
                  {MONTHS[selectedMonth].slice(0,3)} {selectedYear} ▾
                </button>
                {showMonthPick&&(
                  <div style={{position:'absolute',right:0,top:28,background:t.card,borderRadius:12,
                    boxShadow:`0 8px 30px ${t.shadow}`,padding:8,zIndex:50,
                    display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:4,minWidth:190}}>
                    {MONTHS.map((m,i)=>(
                      <button key={i} onClick={()=>{setSelectedMonth(i);setShowMonthPick(false)}}
                        style={{padding:'5px 6px',borderRadius:8,border:'none',cursor:'pointer',
                          fontSize:10,fontWeight:600,
                          background:selectedMonth===i?t.purple:'transparent',
                          color:selectedMonth===i?'white':t.text}}>
                        {m.slice(0,3)}
                      </button>
                    ))}
                  </div>
                ...
                <AreaChart data={chartData} margin={{top:5,right:5,left:-25,bottom:0}}>
                  <defs>
                    <linearGradient id="pinkGrad" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%"  stopColor={t.pink}   stopOpacity={0.25}/>
                      <stop offset="95%" stopColor={t.purple} stopOpacity={0.05}/>
                    </linearGradient>
                  </defs>
                  <CartesianGrid strokeDasharray="3 3" stroke={t.border} vertical={false}/>
                  <XAxis dataKey="date" tick={{fontSize:8,fill:t.subtext}} axisLine={false} tickLine={false} interval={1}/>
                  <YAxis tick={{fontSize:8,fill:t.subtext}} axisLine={false} tickLine={false}
                    tickFormatter={v=>`${v}%`} domain={[0,100]} ticks={[0,50,100]}/>
                  <Tooltip contentStyle={{borderRadius:10,border:'none',boxShadow:`0 4px 20px ${t.shadow}`,
                    background:t.card,color:t.text,fontSize:10}}
                    formatter={v=>[`${v}%`,'Progress']} labelFormatter={l=>`${MONTHS[selectedMonth]} ${l}`}
                    labelStyle={{fontWeight:700,color:t.text}}/>
                  <Area type="monotone" dataKey="productivity" stroke={t.pink} strokeWidth={2} fill="url(#pinkGrad)"
                    dot={false} activeDot={{r:4,fill:t.amber,stroke:t.pink,strokeWidth:2}}/>
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          {/* Bowl */}
          <div style={{...C,padding:12,display:'flex',flexDirection:'column',alignItems:'center',
            justifyContent:'center',height:isMobile?'auto':220}}>
            <p style={{fontWeight:700,fontSize:11,color:t.text,margin:'0 0 2px 0',alignSelf:'flex-start'}}>Overall</p>
            <p style={{fontSize:9,color:t.subtext,margin:'0 0 4px 0',alignSelf:'flex-start'}}>{MONTHS[selectedMonth]} {selectedYear}</p>
            <WaterBowl percent={selectedMonthProgress} t={t}/>
          </div>

          {/* Tasks */}
          <div style={{...C,padding:14,height:isMobile?'auto':220,display:'flex',flexDirection:'column'}}>
            <p style={{fontWeight:700,fontSize:12,color:t.text,margin:'0 0 8px 0',flexShrink:0}}>⚡ Extra Task of the Day</p>
                ...
                {tasks.map((task)=>(
                  <motion.div key={task.id} initial={{opacity:0,y:-5}} animate={{opacity:1,y:0}} exit={{opacity:0}}
                    ...
                    <button onClick={()=>toggleTask(task)}
                      style={{width:18,height:18,borderRadius:4,border:`2px solid ${task.done?t.pink:'#CBD5E1'}`,
                        background:task.done?t.pink:'transparent',cursor:'pointer',flexShrink:0,
                        display:'flex',alignItems:'center',justifyContent:'center'}}>
                    ...
                    <input value={task.title} onChange={e=>updateTaskTitle(task,e.target.value)}
                      ...
                      style={{flex:1,outline:'none',background:'transparent',border:'none',
                        fontSize:11,color:task.done?t.subtext:t.text,
                        textDecoration:task.done?'line-through':'none'}}/>
                    <button onClick={()=>deleteTask(task.id)}
                      style={{background:'none',border:'none',cursor:'pointer',fontSize:13,color:t.subtext,
            ...
            <div style={{flexShrink:0,marginTop:8}}>
              <button onClick={addTask}
                style={{width:'100%',height:tasks.length===0?90:36,display:'flex',alignItems:'center',
                  justifyContent:'center',borderRadius:10,border:`2px dashed ${t.pink}`,
                  background:'transparent',cursor:'pointer'}}>
                <span style={{fontSize:22,color:t.pink}}>+</span>
              </button>
              <p style={{fontSize:9,color:t.subtext,textAlign:'center',marginTop:4,marginBottom:0}}>
                Today only · removed tomorrow
              </p>
            </div>
          </div>
        </div>
        ...
        <div style={{...C,padding:isMobile?12:16,overflowX:'auto'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
            <h3 style={{fontWeight:700,fontSize:15,color:t.text,margin:0}}>Habit Tracker</h3>
            <div style={{display:'flex',alignItems:'center',gap:6}}>
              <button onClick={()=>setHalfOffset(0)}
                style={{padding:'4px 10px',borderRadius:20,border:'none',cursor:'pointer',
                  fontSize:10,fontWeight:600,
                  background:halfOffset===0?t.purple:t.purpleLight,
                  color:halfOffset===0?'white':t.purple}}>1–15</button>
              <button onClick={()=>setHalfOffset(1)}
                style={{padding:'4px 10px',borderRadius:20,border:'none',cursor:'pointer',
                  fontSize:10,fontWeight:600,
                  background:halfOffset===1?t.purple:t.purpleLight,
                  color:halfOffset===1?'white':t.purple}}>16–{daysInSelectedMonth}</button>
            </div>
          </div>

          <div style={{minWidth:isMobile?480:'auto'}}>
            {/* Header */}
            <div style={{display:'flex',alignItems:'center',gap:4,paddingBottom:8,borderBottom:`1px solid ${t.border}`}}>
              <div style={{width:isMobile?90:160,fontSize:11,fontWeight:600,color:t.subtext,flexShrink:0}}>Habit</div>
              <div style={{display:'flex',gap:3,flex:1}}>
                {weekDates.map((d,i)=>(
                  <div key={i} style={{flex:1,textAlign:'center'}}>
                    <p style={{fontSize:isMobile?7:9,fontWeight:600,
                      color:d.date===today?t.pink:t.subtext,margin:0}}>{d.label}</p>
                  </div>
                ))}
              </div>
              <div style={{width:isMobile?56:90,fontSize:11,fontWeight:600,color:t.subtext,
                textAlign:'right',flexShrink:0}}>%</div>
            </div>
              ...
              {habits.map((habit,hi)=>{
                ...
                return (
                  <motion.div key={habit.id} initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} exit={{opacity:0}}
                    style={{display:'flex',alignItems:'center',gap:4,padding:'8px 0',
                      ...
                      {editingHabit===habit.id ? (
                        <input autoFocus value={editName}
                          ...
                          style={{flex:1,outline:'none',border:'none',
                            borderBottom:`1.5px solid ${t.pink}`,
                            background:'transparent',fontSize:isMobile?9:11,
                            fontWeight:600,color:t.text,padding:'1px 2px'}}/>
                      ):(
                        <span onClick={()=>startEdit(habit)} title="Click to rename"
                          style={{fontSize:isMobile?9:11,fontWeight:600,color:t.text,flex:1,
                        ...
                        return (
                          <div key={day.date} style={{flex:1,display:'flex',justifyContent:'center'}}>
                            <button onClick={()=>toggleHabit(habit.id,day.date)}
                              style={{width:isMobile?14:17,height:isMobile?14:17,borderRadius:3,
                                border:`2px solid ${done?t.pink:(isToday?t.purple:'#CBD5E1')}`,
                                background:done?`linear-gradient(135deg,${t.pink},${t.purple})`:'transparent',
                                cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',
                                boxShadow:isToday&&!done?`0 0 0 2px ${t.pinkLight}`:'none',
                    ...
                    {/* Progress */}
                    <div style={{width:isMobile?56:90,display:'flex',alignItems:'center',gap:4,
                      flexShrink:0,justifyContent:'flex-end'}}>
                      {!isMobile&&(
                        <div style={{flex:1,height:3,background:t.purpleLight,borderRadius:99}}>
                          <div style={{height:3,width:`${halfProg}%`,
                            background:`linear-gradient(90deg,${t.pink},${t.purple})`,
                            borderRadius:99,transition:'width 0.5s'}}/>
                        </div>
                      )}
                      <span style={{fontSize:9,fontWeight:700,color:t.pink}}>{halfProg}%</span>
                      <MiniCircle percent={monthProg} t={t}/>
                    </div>
                  </motion.div>
                )
              })}
          ...
          <div style={{display:'flex',gap:8,marginTop:12}}>
            <input value={newHabit} onChange={e=>setNewHabit(e.target.value)}
              ...
              style={{flex:1,borderRadius:12,padding:'8px 14px',outline:'none',
                background:t.inputBg,border:`1px solid ${t.border}`,color:t.text,fontSize:12}}/>
            <button onClick={addHabit}
              style={{padding:'8px 20px',borderRadius:12,
                background:`linear-gradient(135deg,${t.pink},${t.purple})`,
                color:'white',fontWeight:700,fontSize:12,border:'none',cursor:'pointer'}}>Add</button>
          </div>

          {habits.length===0&&(
            <p style={{textAlign:'center',padding:'20px 0',color:t.subtext,fontSize:12,margin:0}}>

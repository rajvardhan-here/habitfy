    ...
    <div style={{ padding: isMobile ? 12 : 24, overflowY: 'auto', minHeight: '100vh', background: YELLOW, fontFamily: 'Inter, sans-serif' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 12, marginBottom: 20 }}>
        ...
        <div style={{ background: 'rgba(40,94,44,0.12)', backdropFilter: 'blur(10px)', border: '1px solid rgba(40,94,44,0.25)', borderRadius: 30, padding: '5px 16px', width: '100%', textAlign: 'center' }}>
          <p style={{ fontSize: 17, color: GREEN, margin: 0, fontStyle: 'italic', textAlign: 'center', opacity: quoteVisible ? 100 : 0, transition: 'opacity 0.5s ease' }}>{QUOTES[quoteIndex]}</p>
        </div>
      </div>

      <div style={{ display: 'flex', gap: 10, marginBottom: 20, alignItems: 'center', flexWrap: 'wrap' }}>
        <input type="month" value={selectedMonth} onChange={e => setSelectedMonth(e.target.value)} style={{ ...inputStyle, width: 'auto', cursor: 'pointer' }} />
        <button onClick={handlePrint} style={{ padding: '8px 20px', borderRadius: 12, background: GREEN, color: YELLOW, fontWeight: 700, fontSize: 13, border: 'none', cursor: 'pointer' }}>🧾 Print Receipt</button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
        {[
          ...
          { label: isOverBudget ? '⚠️ Over Budget!' : 'Remaining', value: `${isOverBudget ? '-' : ''}₹${Math.abs(remaining).toLocaleString()}`, color: isOverBudget ? '#e74c3c' : GREEN },
        ].map((s, i) => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 20, marginBottom: 20 }}>
        <div style={{ ...C, padding: 16 }}>
          <h3 style={{ fontSize: 13, fontWeight: 700, color: GREEN, margin: '0 0 12px 0' }}>Daily Spending — {new Date(selectedMonth + '-01').toLocaleDateString('en-IN', { month: 'long' })}</h3>
          <ResponsiveContainer width="100%" height={160}>
            <AreaChart data={chartData}>
              <defs><linearGradient id="spendGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={GREEN} stopOpacity={0.25} /><stop offset="95%" stopColor={GREEN} stopOpacity={0} /></linearGradient></defs>
              ...
              <Area type="monotone" dataKey="amount" stroke={GREEN} strokeWidth={2.5} fill="url(#spendGrad)" dot={false} activeDot={{ r: 5, fill: YELLOW, stroke: GREEN, strokeWidth: 2 }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        <div style={{ ...C, padding: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 20, marginBottom: 20 }}>
        <div style={{ ...C, padding: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input value={newBudget} onChange={e => setNewBudget(e.target.value)} placeholder={budget ? `Current: ₹${budget.amount}` : 'Enter amount...'} type="number" style={{ ...inputStyle, flex: 1 }} />
            <button onClick={saveBudget} style={{ padding: '8px 16px', borderRadius: 12, fontSize: 13, fontWeight: 700, color: YELLOW, background: GREEN, border: 'none', cursor: 'pointer' }}>Save</button>
          </div>
          {budget && (
        ...
        <div style={{ ...C, padding: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <input value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="What did you spend on?" style={inputStyle} onKeyDown={e => e.key === 'Enter' && addExpense()} />
            <div style={{ display: 'flex', gap: 8 }}>
              <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} style={{ ...inputStyle, flex: 1 }}>
                {categories.map(c => <option key={c}>{c}</option>)}
              </select>
              <input value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })} placeholder="₹" type="number" style={{ ...inputStyle, width: 340 }} onKeyDown={e => e.key === 'Enter' && addExpense()} />
            </div>
            <button onClick={addExpense} style={{ borderRadius: 12, padding: '10px', fontSize: 13, fontWeight: 700, color: YELLOW, background: GREEN, border: 'none', cursor: 'pointer' }}>Add Expense</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ ...C, padding: 16 }} ref={printRef}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <h3 style={{ fontSize: 13, fontWeight: 700, color: GREEN, margin: 0 }}>Recent Expenses ({new Date(selectedMonth + '-01').toLocaleDateString('en-IN', { month: 'long' })})</h3>
          <span style={{ fontSize: 12, color: '#4a7c4e', fontWeight: 600 }}>{monthExpenses.length} entries</span>
        </div>
        {monthExpenses.length === 0 && <p style={{ fontSize: 13, textAlign: 'center', padding: '16px 0', color: '#4a7c4e', margin: 0 }}>No expenses this month!</p>}
        {monthExpenses.map((e) => (
